...
st.markdown("### High-Performance OCR with PDF Support")
st.markdown("---")
...
with col1:
    ...
    if input_type == "Upload Image":
        ...
        if not PDF_SUPPORT:
            st.warning("⚠️ PDF support requires pdf2image")
            st.code("pip install pdf2image")
            st.info("Also install poppler: https://github.com/oschwartz10612/poppler-windows/releases/")
        else:
            pdf_file = st.file_uploader(
                "Upload a PDF (max 50MB, 100 pages)",
                type=["pdf"]
            )
            if pdf_file:
                pdf_mode = True
                        ...
                        with col2:
                            ...
                            for i, img in enumerate(images[:max_pages]):
                                ...
                                if result.get('success'):
                                    with st.expander(f"Page {i+1} Output"):
                                        render_result(result['output'], task_type)
